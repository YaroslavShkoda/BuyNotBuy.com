 ...
 * repository that opens an outbound connection — the Binance transport is the
 * only such path, and it calls `fetch`.
 *
...
beforeEach(() => {
    ...
    replaceGlobally(connectNet, 'connect' as never, () => refuse([]));
    replaceGlobally(connectTls, 'connect' as never, () => refuse([]));
    replaceGlobally(httpRequest, 'request' as never, () => refuse([]));
    replaceGlobally(httpsRequest, 'request' as never, () => refuse([]));
    replaceGlobally(dnsLookup, 'lookup' as never, () => refuse([]));
});
